...
import { Search, Heart, Building2, MapPin } from 'lucide-react';
...
export default function HomePage() {
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col">
      {/* Navbar — logo + auth only, no search */}
      <AppNavbar />

      <main className="flex-1">
        {/* Hero — search bar stays here like original */}
        <section className="bg-[#f7f7f7] py-20 px-6">
          <div className="max-w-3xl mx-auto text-center">
            ...
            <div className="flex items-center bg-white border border-[#dddddd] rounded-[32px] shadow-[0_2px_6px_rgba(0,0,0,0.04)] overflow-hidden max-w-2xl mx-auto">
              ...
              <div className="px-5 py-3.5 border-r border-[#dddddd] text-left shrink-0">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-8">
            {LISTINGS.map((listing) => (
              <div key={listing.id} className="cursor-pointer group">
                <div className={`aspect-[4/3] ${listing.bg} rounded-[14px] overflow-hidden mb-3 relative`}>
      ...
      <footer className="border-t border-[#dddddd] py-8 px-6">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-6">
            <Link href="/terms" className="text-sm font-medium text-[#6a6a6a] hover:text-[#222222] transition-colors">Điều khoản</Link>
            <Link href="/privacy" className="text-sm font-medium text-[#6a6a6a] hover:text-[#222222] transition-colors">Bảo mật</Link>
          </div>
        </div>
      </footer>
    </div>
