    ...
    <div className="space-y-4">
      ...
      >
        ...
        <p className="text-sm font-semibold text-ink-black mb-0.5">
          {dragOver ? 'Thả ảnh vào đây' : 'Kéo thả hoặc nhấn để chọn ảnh'}
        </p>
        <p className="text-xs text-ash-gray text-center">
          JPEG, PNG, WebP · Tối đa {MAX_SIZE_MB}MB mỗi ảnh · Tối đa {MAX_FILES} ảnh
...
export default function NewListingPage() {
  ...
  const onSubmit = (values: FormValues) => {
    ...
    const readyImages = images.filter((i) => !i.uploading && !i.error && i.publicId);
    if (readyImages.length === 0) { setSubmitError('Vui lòng tải lên ít nhất 1 ảnh'); return; }
    ...
    createProperty(payload, {
      onSuccess: () => router.push('/hosting/listings'),
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit as Parameters<typeof handleSubmit>[0])} className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold text-ink-black">Đăng tin mới</h1>
          <p className="text-sm text-ash-gray mt-0.5">Điền đầy đủ thông tin để thu hút người thuê</p>
        </div>
      </div>
      ...
      <SectionCard title="Ảnh bất động sản">
        <p className="text-sm text-ash-gray mb-4">
          Ảnh chất lượng cao giúp tin đăng nổi bật. Ảnh đầu tiên sẽ là ảnh hiển thị chính.
        </p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            >
              <div className="relative">
                ...
                />
                <span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-ash-gray font-medium">
                  ₫/tháng
                </span>
              </div>
            </InputField>
      ...
      <SectionCard title="Mô tả & Tiện nghi">
        <div className="space-y-5">
          <InputField
            ...
            hint="Mô tả chi tiết về phòng, vị trí, xung quanh, quy định..."
          >
            <textarea
              {...register('description')}
              rows={4}
              placeholder="Phòng thoáng mát, sạch sẽ, an ninh tốt. Gần trường học, chợ, bệnh viện..."
      ...
      <div className="sticky bottom-0 -mx-8 px-8 py-4 bg-white border-t border-hairline-gray flex items-center justify-between gap-4">
        {submitError ? (
          ...
          <p className="text-sm text-ash-gray">
            {images.filter((i) => !i.uploading && !i.error).length} ảnh đã tải lên
